...
const ROUTE_PRESETS: RoutePreset[] = [
  ...
  },
  {
    ...
    waypoints: [
      ...
      { name: 'Kawah Bromo Summit (+2329m)', icon: '🌋', x: 340, y: 140, type: 'climb' },
...
const detectedPresetId = computed(() => {
  ...
  if (noteLower.includes('cisadane') || noteLower.includes('tangerang') || props.initialDistance < 15) {
...
function handlePhotoUpload(event: Event) {
  ...
  reader.onload = (e) => {
    if (typeof e.target?.result === 'string') {
      ...
      toast.success('Foto Berhasil Dipasang!', 'Foto dianalisis oleh AI Vision saat meracik cerita.');
    }
  };
...
async function generateAiStory() {
  ...
  try {
    ...
    toast.success('✨ Cerita AI Gemini Dihasilkan!', 'Judul & caption berhasil diperbarui secara cerdas.');
  } catch {
            ...
            >
              ...
              <template v-if="rideForm.aspectRatio === 'landscape'">
                <div class="m-banner-strava-shell">
                  ...
                  <div v-if="rideForm.showGpsRoute && currentRoute" class="m-banner-seamless-map">
                    <svg viewBox="0 0 400 320" class="m-gps-svg" fill="none" aria-hidden="true">
                      <defs>
                        <linearGradient id="mSpectrumL" x1="0%" y1="0%" x2="100%" y2="100%">
                      ...
                      <path :d="currentRoute.pathD" fill="none" stroke="#38BDF8" stroke-width="14" stroke-linecap="round" stroke-linejoin="round" opacity="0.4" />
                      <path :d="currentRoute.pathD" fill="none" stroke="#070D18" stroke-width="9" stroke-linecap="round" stroke-linejoin="round" />
                      <path :d="currentRoute.pathD" fill="none" :stroke="rideForm.routeRenderStyle === 'spectrum_elevation' ? 'url(#mSpectrumL)' : (rideForm.routeRenderStyle === 'kinetic_neon' ? '#C9F36A' : '#38BDF8')" stroke-width="4.5" stroke-linecap="round" stroke-linejoin="round" />
                    ...
                    <div class="m-banner-floating-loc">
                      📍 {{ currentRoute?.name || 'Rute Gowes' }} · <span class="text-sky">+{{ rideForm.elevationM }}m</span>
                    </div>
                  </div>
                  ...
                  <div class="m-banner-seamless-data">
                    <div class="m-banner-top-row">
                      <div class="m-brand-chip-seamless">
                        ...
                        <span class="m-brand-text-seamless">Gowes<span class="brand-kit-accent">Kit</span></span>
                      </div>
                      <div v-if="rideForm.activeSticker !== 'none'" class="m-sticker-chip-seamless">
                    ...
                    <div class="m-banner-telemetry-seamless">
                      <div class="mb-stat-block">
                        ...
                        <strong class="mb-stat-val">{{ formatDuration(rideForm.durationMinutes) }}</strong>
                      </div>
                      <div class="mb-stat-block">
                        <span class="mb-stat-lbl">SPEED</span>
                        <strong class="mb-stat-val text-lime">{{ rideForm.avgSpeedKmH }} km/h</strong>
                      </div>
                      <div class="mb-stat-block">
                        <span class="mb-stat-lbl">CLIMB</span>
                        <strong class="mb-stat-val text-sky">+{{ rideForm.elevationM }}m</strong>
                      </div>
                      <div class="mb-stat-block">
                        <span class="mb-stat-lbl">KALORI</span>
                        <strong class="mb-stat-val text-coral">~{{ rideForm.caloriesKcal }}</strong>
                      </div>
                    </div>
              ...
              <template v-else-if="rideForm.aspectRatio === 'post'">
                <div class="m-square-shell">
                  ...
                  <div class="m-square-map-stage">
                    <div v-if="rideForm.showGpsRoute && currentRoute" class="m-square-svg-wrap">
                      <svg viewBox="0 0 400 320" class="m-gps-svg" fill="none" aria-hidden="true">
                        <defs>
                          <linearGradient id="mSpectrumS" x1="0%" y1="0%" x2="100%" y2="100%">
                            <stop offset="0%" stop-color="#C9F36A" />
                            <stop offset="35%" stop-color="#8EDDF4" />
                            <stop offset="70%" stop-color="#F59E0B" />
                            <stop offset="100%" stop-color="#FF8C75" />
                        ...
                        <path :d="currentRoute.pathD" fill="none" stroke="#38BDF8" stroke-width="14" stroke-linecap="round" stroke-linejoin="round" opacity="0.4" />
                        <path :d="currentRoute.pathD" fill="none" stroke="#070D18" stroke-width="9" stroke-linecap="round" stroke-linejoin="round" />
                        <path :d="currentRoute.pathD" fill="none" :stroke="rideForm.routeRenderStyle === 'spectrum_elevation' ? 'url(#mSpectrumS)' : (rideForm.routeRenderStyle === 'kinetic_neon' ? '#C9F36A' : '#38BDF8')" stroke-width="4.5" stroke-linecap="round" stroke-linejoin="round" />
                  ...
                  <div class="m-square-bottom">
                    ...
                    <div class="m-square-metrics">
                      <div class="msq-pill">
                        ...
                        <strong class="msq-val">{{ formatDuration(rideForm.durationMinutes) }}</strong>
                      </div>
                      <div class="msq-div"/>
                      <div class="msq-pill">
                        <span class="msq-lbl">AVG SPEED</span>
                        <strong class="msq-val text-lime">{{ rideForm.avgSpeedKmH }} km/h</strong>
                      </div>
                      <div class="msq-div"/>
                      <div class="msq-pill">
                        <span class="msq-lbl">CLIMB</span>
                        <strong class="msq-val text-sky">+{{ rideForm.elevationM }}m</strong>
                      </div>
                      <div class="msq-div"/>
                      <div class="msq-pill">
                        <span class="msq-lbl">KALORI</span>
                        <strong class="msq-val text-coral">~{{ rideForm.caloriesKcal }}</strong>
                      </div>
                    </div>
                  </div>
              ...
              <template v-else>
                ...
                <div class="modal-poster-bot">
                  ...
                  <svg viewBox="0 0 300 20" class="m-elev-curve-svg" aria-hidden="true" style="width: 100%; height: 16px; margin: 0.15rem 0;">
                    <path
                      ...
                      stroke-linecap="round"
                    />
                    <circle cx="0" cy="16" r="2" :fill="rideForm.templateStyle === 'cafe_santai' ? '#F59E0B' : '#38BDF8'" />
                    <circle cx="300" cy="2" r="3" :fill="rideForm.templateStyle === 'cafe_santai' ? '#FDE68A' : (rideForm.templateStyle === 'rapha_editorial' ? '#FFD1C9' : '#C9F36A')" />
                  </svg>
                  <div class="m-pillars-row">
                    <div class="m-pcol">
                      <span class="m-plbl">WAKTU</span>
                      <strong class="m-pval">{{ formatDuration(rideForm.durationMinutes) }}</strong>
                    </div>
                    <div class="m-pdiv" />
                    <div class="m-pcol">
                      <span class="m-plbl">SPEED</span>
                      <strong class="m-pval" :class="rideForm.templateStyle === 'cafe_santai' ? 'text-amber' : (rideForm.templateStyle === 'rapha_editorial' ? 'text-coral' : 'text-lime')">{{ rideForm.avgSpeedKmH }} km/h</strong>
                    </div>
                    <div class="m-pdiv" />
                    <div class="m-pcol">
                      <span class="m-plbl">KALORI</span>
                      <strong class="m-pval text-coral">~{{ rideForm.caloriesKcal }}</strong>
                    </div>
                  </div>
                </div>
